...
void GradientCalculator::calculateGradient(cv::Mat &src) {
	...
	if (src.rows>1) {
		...
		for (x = 0; x < src.cols; x++) {
			...
			gradX.at<float>(0, x) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);

			pixel1 = src.at<cv::Vec3b>(src.rows - 1, x);
			pixel0 = src.at<cv::Vec3b>(src.rows - 2, x);
			pixelDiff = pixel1 - pixel0;
			gradX.at<float>(src.rows - 1, x) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);
	...
	if (src.rows>2) {
		for (y = 1; y < src.rows - 1; y++) {
			for (x = 0; x < src.cols; x++) {
				...
				pixelDiff = pixel1 - pixel0;
				gradX.at<float>(y, x) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);
	...
	if (src.cols>1) {
		...
		for (y = 0; y < src.rows; y++) {
			...
			pixelDiff = pixel1 - pixel0;
			gradY.at<float>(y, 0) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);

			pixel1 = src.at<cv::Vec3b>(y, src.cols - 1);
			pixel0 = src.at<cv::Vec3b>(y, src.cols - 2);
			pixelDiff = pixel1 - pixel0;
			gradY.at<float>(y, src.cols - 1) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);
	...
	if (src.cols>2) {
		for (x = 1; x < src.cols - 1; x++) {
			for (y = 0; y < src.rows; y++) {
				...
				pixelDiff = pixel1 - pixel0;
				gradY.at<float>(y, x) = -(pixelDiff[0] + pixelDiff[1] + pixelDiff[2]) / (3.f*255.0f);
