...
int main(int argc, char *argv[])
{
	...
	InftamRgbdSlam slam(depthCamModel, depthCamModel, mat4::Identity());
	...
	while(running) {
		...
		depthCam.getLatestFrames();
		slam.processFrames(depthCam.colorFramePtr(), depthCam.depthFramePtr());
		...
		cv::Mat realDepth(cv::Size(depthCam.colorWidth(), depthCam.colorHeight()),
			CV_16UC1, slam.expectedDepthMap());
		depthTexture.update(realDepth.data);
		virtualDepthTexture.texture().getData(glDepth.data, glColor.rows*glColor.cols * 2);
		cv::flip(glDepth, glDepth, 0);
		matte = (realDepth > glDepth) & (glDepth != 0);
